...
class MemberChatConstants {
  ...
  static const String appTitle = '회원 채팅';
  static const String chatHistoryKey = 'member_chat_history';
...
class MemberChatScreen extends StatefulWidget {
  ...
  @override
  State<MemberChatScreen> createState() => MemberChatScreenState();
}

class MemberChatScreenState extends State<MemberChatScreen> {
  final TextEditingController _messageController = TextEditingController();
  final List<ChatMessage> _messages = [];
  final MemberChatService _chatService = MemberChatService();
  ...
  Future<void> _loadChatHistory() async {
    ...
    try {
      final chatHistory = _prefs!.getString(MemberChatConstants.chatHistoryKey);
      if (chatHistory != null) {
  ...
  Future<void> _saveChatHistory() async {
    ...
    try {
      ...
      await _prefs!.setString(MemberChatConstants.chatHistoryKey, messagesJson);
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> _sendMessage() async {
    ...
    setState(() {
      _messages.add(
        ChatMessage(content: userMessage, role: MemberChatConstants.userRole),
      );
      _messages.add(
        ChatMessage(
          content: '답변을 생성하는 중...',
    ...
    try {
      ...
      if (mounted) {
        ...
        _showToast('${MemberChatConstants.errorMessage}$e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(MemberChatConstants.appTitle),
