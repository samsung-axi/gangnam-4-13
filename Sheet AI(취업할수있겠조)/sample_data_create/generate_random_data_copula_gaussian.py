...
#%%
class MultivariateCopulaGenerator:
    def __init__(self, df_multiindex):
        """
        MultiIndex DataFrame을 입력받아 초기화
        Copula를 사용한 다변량 정규분포 기반 데이터 생성
        """
    ...
    def _fit_empirical_distributions(self, credit_grade):
        """각 지표별 경험적 분포 함수 학습"""
        ...
        for metric in self.financial_metrics:
            try:
                ...
                if not (pd.isna(mean_val) or pd.isna(std_val)):
                    # 정규분포 기반의 경험적 분포 함수 생성
                    self.empirical_cdfs[metric] = lambda x, m=mean_val, s=std_val: norm.cdf(x, m, s)
                    self.empirical_ppfs[metric] = lambda x, m=mean_val, s=std_val: norm.ppf(x, m, s)
            except KeyError:
                try:
    ...
    def generate_company_data(self, credit_grade, company_name=None, seed=None, 
        ...
        # 경험적 분포 함수 학습
        self._fit_empirical_distributions(credit_grade)
        ...
        for attempt in range(max_attempts):
            ...
            for i, metric in enumerate(valid_metrics):
                if metric in self.empirical_ppfs:
                    lower_bound = self.empirical_ppfs[metric](percentile_range[0] / 100.0)
                    upper_bound = self.empirical_ppfs[metric](percentile_range[1] / 100.0)
