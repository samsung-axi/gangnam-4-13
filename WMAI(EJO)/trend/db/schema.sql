-- TrendStream 데이터베이스 스키마
-- MySQL 8.4.6 LTS

-- 데이터베이스 생성 (필요시)
CREATE DATABASE IF NOT EXISTS trendstream
...
-- 원시 이벤트 팩트 테이블
-- 참고: 파티셔닝은 제거되었습니다 (MySQL 8.4 호환성 문제)
...
-- 쓰기 권한 사용자
CREATE USER IF NOT EXISTS 'trendstream_writer'@'%' IDENTIFIED BY 'trendstream123';
GRANT SELECT, INSERT, UPDATE ON trendstream.* TO 'trendstream_writer'@'%';

-- 읽기 전용 사용자
CREATE USER IF NOT EXISTS 'trendstream_reader'@'%' IDENTIFIED BY 'readonly123';
GRANT SELECT ON trendstream.* TO 'trendstream_reader'@'%';
