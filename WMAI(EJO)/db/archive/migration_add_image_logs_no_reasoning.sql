-- 이미지 분석 로그 테이블 추가 (reasoning 제외 - 비용 절감)
-- 작성일: 2025-11-11

-- 기존 테이블이 있으면 삭제 (개발용)
DROP TABLE IF EXISTS image_analysis_logs;
DROP VIEW IF EXISTS v_blocked_images;
...
COMMENT='이미지 분석 로그 (NSFW + Vision API, reasoning 제외)';

-- 차단된 이미지 뷰 (관리자용)
CREATE VIEW v_blocked_images AS
SELECT 
    l.id,
    l.filename,
    l.original_name,
...
-- 마이그레이션 완료
SELECT 'Migration completed: image_analysis_logs table created (without reasoning)' AS status;
