...
resource "aws_cloudwatch_metric_alarm" "sqs_high_depth" {
  ...
  statistic           = "Average"
  threshold           = 50
  alarm_description   = "SQS 대기 메시지 50개 초과 (3분)"

  dimensions = { QueueName = aws_sqs_queue.analysis.name }
...
resource "aws_cloudwatch_metric_alarm" "dlq_messages" {
  ...
  statistic           = "Sum"
  threshold           = 0
  alarm_description   = "DLQ에 메시지 유입 (분석 3회 실패)"
...
resource "aws_cloudwatch_metric_alarm" "ec2_high_cpu" {
  ...
  statistic           = "Average"
  threshold           = 85
  alarm_description   = "ECS EC2 인스턴스 평균 CPU 85% 초과"
