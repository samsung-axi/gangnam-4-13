...
def backtest_tcn(test_year: int = 2024, weights_suffix: str = "") -> dict:
    ...
    # weights_suffix가 있으면 파일명에 추가 (예: run2 → _run2 → finetuned_mapo_tcn_run2.pt)
    # 없으면 기존 기본 파일명 그대로 사용
    _sfx = f"_{weights_suffix}" if weights_suffix else ""
    weights_path = WEIGHTS_DIR / f"finetuned_mapo_tcn{_sfx}.pt"
    scalers_path = WEIGHTS_DIR / f"finetune_tcn_scalers{_sfx}.pkl"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="가중치/스케일러/CSV 파일명 suffix (예: run2 → finetuned_mapo_tcn_run2.pt, tcn_backtest_results_run2.csv)",
    )
    args = parser.parse_args()

    # 백테스트 실행 — weights_suffix 전달 (기본값 "" = 기존 동작 유지)
    result = backtest_tcn(test_year=args.year, weights_suffix=args.weights_suffix)
    ...
    if "error" not in result:
        ...
        # 없으면 기본 파일명(tcn_backtest_results.csv) 그대로 사용
        _sfx = f"_{args.weights_suffix}" if args.weights_suffix else ""
        results_csv = RESULTS_DIR / f"tcn_backtest_results{_sfx}.csv"
