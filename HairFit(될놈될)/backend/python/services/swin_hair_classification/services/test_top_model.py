...
def load_top_model(device):
    """Top 모델 로드"""
    model = SwinHairClassifier(num_classes=4)
    model_path = 'models/best_swin_hair_classifier_top.pth'
    ...
    if os.path.exists(model_path):
        ...
        print(f"Top 모델 로드 완료: {model_path}")
    else:
        print(f"경고: Top 모델 파일을 찾을 수 없습니다: {model_path}")
        return None
...
def test_top_model(image_path):
    ...
    try:
        ...
        print(f"Top 모델 분류 실패: {e}")
        return None, None

def main():
    """메인 함수"""
    print("=== Top 모델 전용 탈모 분석기 ===")
    print("위/정면 이미지(Top/Front) 분석에 특화된 모델입니다.")
