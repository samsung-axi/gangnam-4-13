...
class ChatService(
  ...
  suspend fun combineMessages(userId: String, chat: ChatRoom, newMessage: String): List<Message> {
    ...
    return history.map { Message(role = it.role, content = it.content) }
      .toMutableList()
      .apply { add(Message(role = "user", content = newMessage)) }
  }

  suspend fun saveMessage(userId: String, chatId: String, userMsg: String, aiMsg: ChatResult) {
    messageRepository.save(userId, chatId, Message(role = "user", content = userMsg))
    messageRepository.save(userId, chatId, Message(role = "assistant", content = aiMsg.answer))
